...
Df = yf.download('AUDCHF=X', start_date, today, interval='5M', auto_adjust=True)
...
Df = Df.tz_localize('UTC').tz_convert('America/Sao_Paulo')
Df = Df.tz_localize(None)
...
# Collect data from Yahoo (hourly)
data = yf.download('AUDCHF=X', start_date, today, interval='5M', auto_adjust=True)

# Setting Timezone from UTC to US/Pacific
data = data.tz_localize(None)
data = data.tz_localize('UTC').tz_convert('America/Sao_Paulo')
data = data.tz_localize(None)
...
# Print the forecast
data.tail(5)[['signal','predicted_BTC_price']].T
...
df2 = df.loc[:, ["Datetime","Price_prediction", "Difference"]]

##Add columns and constant values

df2['AUD_NAM'] = 'AUD_CHF'
df2['AUD_ALG'] = 'Script4_ML-LinearRegression'
df2['AUD_CAT'] = 'Forex'

df2 = df2.loc[:, ['Datetime', 'Price_prediction', 'Difference', 'AUD_NAM', 'AUD_ALG', 'AUD_CAT']].copy()
df2.columns = ['AUD_DATETIME_PRED', 'AUD_PRICE_PRED', 'AUD_DIF', 'AUD_NAM', 'AUD_ALG', 'AUD_CAT']
df2['AUD_DATETIME_PRED'] = df2['AUD_DATETIME_PRED'] + pd.Timedelta(hours=0, minutes=5, seconds=0)
...
df2.to_sql(con=my_conn,name='AUD_CHF',if_exists='append',index=False)


## The best: https://www.statology.org/pandas-add-subtract-time-to-datetime/
## EXCELLENT => https://blog.hubspot.com/website/filter-rows-pandas
